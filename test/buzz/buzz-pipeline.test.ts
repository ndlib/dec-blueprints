...
describe('BuzzPipeline', () => {
  ...
  describe('creates a RailsPipeline', () => {
      ...
      expect(MockedRailsPipeline).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          testStage: {
            ...
            configPath: '/all/test.pipelineProp.namespace-test-buzz',
            ...
            stackname: 'test.pipelineProp.namespace-test-buzz',
    ...
    test('that will push containers to the Buzz ECR from the pipeline foundation', async () => {
      lazyEval.subject
      expect(MockedRailsPipeline).toHaveBeenCalledWith(
        expect.any(Object),
        'DeploymentPipeline',
        expect.objectContaining({
          ecr: lazyEval.pipelineFoundationStack.ecrs.Buzz,
        }),
      )
    })

    test('that creates a rails container with a migration', async () => {
      lazyEval.subject
      expect(MockedRailsPipeline).toHaveBeenCalledWith(
        expect.any(Object),
        'DeploymentPipeline',
